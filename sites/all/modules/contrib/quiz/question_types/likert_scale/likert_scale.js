var LikertScale = LikertScale || {};
(function ($) {

  LikertScale.refreshScores = function (checkbox, scoring) {
    var prefix = '#' + LikertScale.getCorrectIdPrefix(checkbox.id);
    if (checkbox.checked) {
   ...
   */
  LikertScale.refreshCorrect = function (textfield) {
    var prefix = '#' + LikertScale.getCorrectIdPrefix(textfield.id);
    var chosenScore;
    var notChosenScore;

    // Fetch the score if chosen and score if not chosen values for the active alternative
    if (LikertScale.isChosen(textfield.id)) {
   ...
   */
  LikertScale.getCorrectIdPrefix = function (string) {
   ...
   */
  LikertScale.isChosen = function (string) {
    var pattern = new RegExp("score-if-chosen$");
    return pattern.test(string);
  }

  Drupal.behaviors.LikertScaleAlternativeBehavior = {
              ...
              .click(function (event) {
                ...
                if (event.target.type !== 'checkbox') {
                  ...
                  if ($(':radio', this).html() != null) {
                    $(this).parent().find('.LikertScale-row').removeClass('selected');
